...
const algo = {
    ...
    description: (
        <>
            Дана вселенная элементов U и набор подмножеств S₁…Sₙ. Жадный подход на каждом шаге выбирает множество,
            которое покрывает максимум ещё не покрытых элементов, удаляет их из «непокрытых» и повторяет, пока все
            элементы не покрыты или больше нечем покрывать.
        </>
    ),
    complexity: (
        <>
            Наивная оценка: <code>O(k · n · m)</code>, где <i>k</i> – число выбранных множеств, <i>n</i> – всего множеств,
            &nbsp;<i>m</i> – мощность вселенной. Память – <code>O(m)</code> для трекинга непокрытых.
        </>
    ),
