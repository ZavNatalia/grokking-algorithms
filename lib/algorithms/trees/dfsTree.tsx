...
const algo = {
    ...
    description: (
        <>
            Файловая система – дерево: каталоги – узлы, файлы – листья. Обход в
            глубину (depth-first search, DFS) уходит по пути максимально далеко,
            используя стек, затем откатывается назад. Подходит для задач вроде:
            вывести имена всех файлов в каталоге <code>pics</code>, включая
            подкаталоги.
        </>
    ),
    complexity: (
        <>
            Время – <code>O(N)</code> по числу записей; память –{' '}
            <code>O(H)</code>, где <i>H</i> – максимальная глубина.
        </>
    ),
