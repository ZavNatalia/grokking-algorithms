...
const algo = {
    ...
    description: (
        <>
            Идея ДП: разбиваем задачу на подзадачи и запоминаем ответы. Для
            рюкзака строим таблицу
            <code>items × capacity</code>, где ячейка <code>dp[i][w]</code>{' '}
            хранит лучшую стоимость при использовании первых &nbsp;<i>i</i>{' '}
            предметов и вместимости <i>w</i>. Переход: максимум из «не брать
            предмет i» и «взять его, если помещается». В конце восстанавливаем
            состав решения, двигаясь по таблице вверх.
        </>
    ),
    complexity: (
        <>
            Время – <code>O(n·W)</code>, память – <code>O(n·W)</code> (или{' '}
            <code>O(W)</code> с одномерной оптимизацией без восстановления
            решения).
        </>
    ),
