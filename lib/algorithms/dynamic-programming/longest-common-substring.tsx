...
function lcsstrEval(a: string, b: string): { length: number; substring: string } {
...
const algo = {
    ...
    description: (
        <>
            <code>dp[i][j]</code> хранит длину общего суффикса для префиксов <code>a[0..i)</code> и <code>b[0..j)</code>.
            При совпадении символов: <code>dp[i-1][j-1]+1</code>, иначе <code>0</code>. Отслеживаем максимум и конец.
        </>
    ),
    complexity: <>Время – <code>O(n·m)</code>, память – <code>O(n·m)</code>.</>,
