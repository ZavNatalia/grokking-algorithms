...
const algo = {
    ...
    description: (
        <>
            Таблица <code>(n+1)×(m+1)</code>, где <code>dp[i][j]</code> – длина LCS префиксов <code>a[0..i)</code> и <code>b[0..j)</code>.
            При равенстве символов: <code>dp[i-1][j-1]+1</code>, иначе <code>max(dp[i-1][j], dp[i][j-1])</code>.
        </>
    ),
