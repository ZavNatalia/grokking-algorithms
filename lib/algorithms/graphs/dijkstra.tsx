...
const algo = {
    ...
    description: (
        <>
            Алгоритм для ориентированных графов с неотрицательными весами: на каждом шаге выбирает
            непроработанную вершину с минимальной стоимостью (lowest-cost node), пересчитывает соседей
            и помечает вершину обработанной.
        </>
    ),
    complexity: (
        <>
            Время – <code>O(V^2)</code> с линейным поиском узла / <code>O(E&nbsp;log&nbsp;V)</code> с приоритетной очередью; память – <code>O(V)</code>.
        </>
    ),
